...
```{r}
text_field <- read_rds('text_field.rds')

remove_vals <- c('uni')
mapping <- c('Immigration_background' = 'immigration_backgrou')

text_field <- remove(text_field, remove_vals) %>% map_values(mapping)

vars <- colnames(choice_df)
vars <- remove(vars, c(text_field, gen_vars))
vars <- vars[!str_detect(vars, '(^Q7)|(TEXT$)')]

choice_text_removed <- choice_df %>% select(vars) 
numeric_text_removed <- numeric_df %>% select(vars) 
```
...
converted_choice_text_removed <- convert_choiceDF(choice_text_removed, vars)
check_vars(numeric_text_removed, converted_choice_text_removed, vars = vars)
```
